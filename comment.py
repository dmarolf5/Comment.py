# These comments are intended to show competency with using comments.
#! /usr/bin/python
...
def get_glasses(day):
	glasses_count = (10 + 8.8 * (day % 4))/(4 + .3 * day)
	return glasses_count
# I changed glasses_count to return t(n)/c(n) as the problem requested, where t(n), high temperature, was 10 + 8.8(n % 4) and c(n) was cost measured as 4 + 0.3n.

def get_price(day):
	price = 6 + day % 3
	return price
# The price changed from 8 to the new p(n), which was given as 6 + n % 3.

def get_signs(day, assets, sign_cost):
	number_signs = 2 - 2 * ((15 * (day % 5)) / 100)
	return number_signs
# I changed signs, which was equivalent to s_n, to, instead of 1, be 2 - 2r(n).
